...
import { cataloqueLinks, sideBarLinks } from "@/utils/general/siderBarLinks";
...
export default function Sidebar({
  ...
  const styles = " w-full space-x-3 flex items-center capitalize dark:hover:bg-slate-500 hover:bg-slate-200 hover:text-amber-400";
  const activeLinkDefaul =
    " border-amber-400 bg-slate-400 dark:bg-slate-800 text-amber-400 dark:text-slate-100 hover:bg-slate-500";
  const activeLink = `${activeLinkDefaul} border-l-8`;

  return (
    <div
      className={`mt-0 space-y-3  transition-all duration-500 ease-in-out z-20 bg-slate-50 dark:bg-slate-700  h-screen text-slate-500 dark:text-slate-100 fixed left-0 top-0 bottom-0 ${sidebarWidth}`}
    >
      <div className="overflow-y-auto h-screen">
        ...
        >
          ...
          <X className="sm:hidden cursor-pointer" onClick={toggleShowSiderbarCatalogue} />
        </div>
